...
if (isset($_POST['submit'])) {
  ...
  // Cek apakah ISSN sudah ada
  $query_check_issn = "SELECT issn FROM data_buku WHERE issn = ?";
  $stmt_check_issn = $db->prepare($query_check_issn);
  $stmt_check_issn->bind_param("s", $issn);
  $stmt_check_issn->execute();
  $stmt_check_issn->store_result();

  if ($stmt_check_issn->num_rows > 0) {
      // ISSN sudah ada, tampilkan pesan atau popup
      echo "<script>alert('Maaf, data buku dengan ISSN sudah tersedia.');</script>";
  } else {
      // ISSN belum ada, lanjutkan proses penyimpanan
      $cover = '';
  ...
  // Close the statement for ISSN check
  $stmt_check_issn->close();
}
?>
